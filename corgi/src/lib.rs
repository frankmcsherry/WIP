...
pub mod arrange {
    ...
    /// The positions a 0/1 MASK column keeps, at any leaf width: the core's idiom is "nonzero is
    /// true", so any leaf reads. `None` if the column is not a leaf.
    ///
    /// This is what `Op::Filter` does internally, exposed because a host filtering its OWN
    /// parallel columns (times, diffs, a row id) alongside corgi's needs the same index list, and
    /// reaching for the leaf directly makes the host care about a width that is corgi's business.
    pub fn mask_positions(v: &Value) -> Option<Vec<usize>> {
        let Value::Prim(p) = v else { return None };
