...
mod lockstep {
    //! What `Fold` and `FoldScan` need to run a body across rows in lockstep: where the
    //! accumulator's update goes ([`scatter`]), where a round's output goes ([`OutputSink`]), and
    //! which rows are still long enough to take part ([`init_active`]).
    //!
    //! The recurring split is FIXED-WIDTH versus not. A leaf, a product of leaves, or a unit row
    //! occupies a constant slot, so an update can be written straight into it; a `List` or `Sum` row
    //! is a span or a lane position and has no slot, so it is rebuilt by a multi-source gather.
    //! `scatter` makes that split for the accumulator and `OutputSink` makes it for the output
    ...
    impl OutputSink {
        ...
        /// column's length.
        pub(super) fn take(&mut self, elem: &[usize], r: Value, total: usize) {
...
use lockstep::{init_active, scatter, OutputSink};
    ...
    pub(crate) fn eval(&self, input: Value) -> Result<Value, String> {
        ...
        Ok(match self {
            ...
            Op::Filter => {
                ...
                // nonzero-is-true leaf reads the same. One dispatch, above the pass — and no
                // normalizing pass over the mask, which `as_byte_mask` would cost at a wide one.
