...
macro_rules! prim {
    ($($V:ident => $t:ty),+ $(,)?) => {
        ...
        impl Prim {
            ...
            /// The order of `self[lo..hi]` in one pass: `None` at the first inversion, else whether
            /// the range is STRICTLY increasing (no two neighbours equal). Both answers carry: an
            /// inversion settles that a column is unsorted whatever follows the leading leaf, and
            /// strict increase settles that it IS sorted, since a lexicographic order whose
            /// leading component already separates every pair never consults the rest.
