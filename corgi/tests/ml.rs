//! ML front-end tests: same demos through `let`/`|>`/`fun`, plus a check that `let`
...
#[test]
fn match_contact() {
    let src = "input.2 |> map_variant 1 (fun p -> p |> add_u64 1000000) |> unwrap";
...
fn const_via_lit_lambda() {
    let src = "let (subj, vals) = input.1 |> transpose in \
               vals |> map (fun v -> (v, v |> lit 1000) |> add)";
...
fn workhorse_products_sums_lists() {
    ...
    let src = "let scores = input.1 |> transpose in \
               ...
               let contact = input.2 |> map_variant 0 (fun e -> e |> add_u64 1000000) |> unwrap in \
               (id_plus, contact)";
...
fn string_literal_broadcasts() {
    // a string literal applied via `|>` is a constant List<U8>, broadcast to the value.
