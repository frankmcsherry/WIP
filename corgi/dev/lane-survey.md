...
A probe over sorted pairs of columns, master → here:

| shape | master | here |
|---|---|---|
| u64, 2M distinct values | 3.9 | 4.1 |
| (u64 % 200k, u64 % 1000) | 108.8 | 22.4 |
| (u64 % 4M, u64 % 1000) | 48.2 | 30.6 |
| (u64 % 100k, Sum of two u64 lanes % 1000) | 216.3 | 30.3 |
| (u64 % 100k, List<u64> of 0..3 % 1000) | 142.5 | 38.9 |
| Sum of two u64 lanes % 100k | 73.0 | 5.6 |
| List<u64> of 0..3 % 1000 | 68.5 | 22.1 |

Gap rows, `survey` (the pairwise form) against a typed Rust two-pointer over tuples, at 1M and
8M rows a side: R4, the leaf over 256-row runs, 0.061 → 0.064 and 0.219 → 0.215; R11 `(u64, u64)`
38.5 → 16.8 and 38.6 → 13.0 (Rust 1.6 / 1.4); R12 `(u64, Sum of two u64 lanes)` 77.5 → 26.0 and
78.2 → 18.2 (Rust 1.7 / 1.4). So two to three times master on structured columns, and an order
of magnitude still short of the typed walk: what remains is per report, the node the tree holds
for it, the flatten, and the pairwise expansion `survey` adds on top of the groups, which a
consumer of `survey_groups` does not pay. DDIR built against this branch passes its tests.
